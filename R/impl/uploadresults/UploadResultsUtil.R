...
UploadResultsUtil$uploadResults <- function () {
  ...
  resultsFolderRoot <- config$outputDir
  ...
  tryCatch({
    for (i in 1:nrow(dfResultsFolders)) {
      ...
      resultsDatabaseSchema <- paste0(config$resultsSchemaPrefix, resultFolder$analysis)
    ...
    for (analysis in distictAnalyses) {
      resultsDatabaseSchema <- paste0(config$resultsSchemaPrefix, analysis)
